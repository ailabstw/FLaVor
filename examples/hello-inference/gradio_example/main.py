...
from flavor.serve.strategies import AiCOCOInputStrategy, GradioSegmentationStrategy
...
class Inferer:
    ...
    def make_infer_result(
        self, model_out: np.ndarray, sorted_data_filenames: List[str], data: np.ndarray, **kwargs
    ) -> Dict:
    ...
    def infer(self, **kwargs) -> Dict[str, Any]:
        ...
        data = batch.squeeze(0)
        model_out = self.inferer.apply(data)
        ...
        return model_out, data_filenames, data

    def __call__(self, **kwargs):

        model_out, sorted_data_filenames, data = self.infer(**kwargs)
        result = self.make_infer_result(model_out, sorted_data_filenames, data, **kwargs)
