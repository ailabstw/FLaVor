...
- `model_out`:
  ...
  * detection: pack detection results in a dictionary containing key-value pairs of `"bbox_pred"`, `"confidence_score"` and `"regression_value"`.
    * `"bbox_pred"`: a list of bbox predictions. For example, ```[[x_min, y_min, x_max, y_max, cls], ...]```
    * `"confidence_score"`: (optional) a list of confidence scores of each bbox.
    * `"regression_value"`: (optional) a list of regression values of each bbox
