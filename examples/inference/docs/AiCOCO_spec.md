...
The AiCOCO format is described in detail below including required and optional fields for `images`, `annotations`, `categories`, `regression` and `objects`. Please replace all placeholders with actual values, e.g.,  `<filename>.<ext>`, `nanoid`, and details under each JSON key.
